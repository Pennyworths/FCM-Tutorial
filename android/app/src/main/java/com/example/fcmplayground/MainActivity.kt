...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Handle notification click: extract nonce from Intent and send ACK
        // This handles the case when app is in background or closed
        // FCM automatically includes data fields in Intent extras when notification is clicked
        handleNotificationClick(intent, apiBaseUrl)
    ...
    /**
     * Handle notification click: extract nonce from Intent and send ACK.
     * This is called when user clicks a notification while app is in background or closed.
     * FCM automatically includes message data fields in Intent extras when notification is clicked.
     * 
     * When app is in background/closed:
     * - FCM displays notification automatically (if message has notification field)
     * - onMessageReceived is NOT called
     * - When user clicks notification, FCM puts data fields in Intent extras (keys are original data keys)
     */
    private fun handleNotificationClick(intent: android.content.Intent?, apiBaseUrl: String) {
        ...
        if (nonce.isNullOrBlank()) {
            ...
            if (keys.isNotEmpty()) {
                Log.d(TAG, "Notification clicked but no nonce found. Intent extras keys: $keys")
        ...
        val messageType = extras.getString("type")
            ?: extras.getString("gcm.type")
            ?: extras.getString("fcm.type")
            ?: null

        val endpoint = if (messageType == "e2e_test") {
            ApiRoutes.TEST_ACK
        } else {
            ApiRoutes.MESSAGES_ACK
        }

        Log.d(TAG, "Notification clicked: nonce=$nonce, type=$messageType, sending ACK to $endpoint")
