...
main() {
    ...
    log "Testing connection from instance..."
    local command_id=$(aws ssm send-command \
        --region "$AWS_REGION" \
        --instance-ids "$INSTANCE_ID" \
        --document-name "AWS-RunShellScript" \
        --parameters "commands=[\"PGPASSWORD='$DB_PASSWORD' psql -h $RDS_ENDPOINT -p 5432 -U $DB_USERNAME -d $DB_NAME -c 'SELECT version();'\"]" \
    ...
    if [[ "$status" == "Success" && -n "$output" ]]; then
